...
\begin{frame}[fragile]
\frametitle{Why llama.cpp? Not Ollama?}

\href{https://ollama.com/}{Ollama} is great for quick experiments, but \href{https://github.com/ggml-org/llama.cpp}{llama.cpp} offers more control for production use cases.

\begin{itemize}
\item \textbf{Performance}: Faster than Ollama for raw inference.
\item \textbf{Concurrency}: Better support for concurrent requests.
\item \textbf{Flexibility}: Granular configuration (many settings are hidden in Ollama).
\item \textbf{Quantization}: Better control (Ollama's quantized versions can behave oddly).
\end{itemize}
\end{frame}
\begin{frame}[fragile]
...
\item Higher-precision (e.g., Q8\_0) preserves accuracy better but is slower and uses more memory.
\item Lower-precision (e.g., Q4\_K\_S or Q2) is faster but can reduce accuracy.
\end{itemize}
...
\begin{frame}[fragile]
\frametitle{Installing Python Dependencies}

\begin{minted}{bash}
pip install openai fastmcp httpx
\end{minted}
\end{frame}
...
\begin{frame}
\frametitle{Tools/Function Calling vs. MCP}
...
\hline
\textbf{Feature} & \textbf{Tools / Function Calling} & \textbf{Model Context Protocol (MCP)} \\
\hline
\textbf{Role} & The capability to execute code. & The standard for connecting tools to AIs. \\
\textbf{Integration} & \textbf{1-to-1:} Custom code for each model/app. & \textbf{M-to-N:} One server connects to \textit{any} client. \\
\textbf{Portability} & Low. Rewriting often needed for different schemas. & High. Protocol abstracts model differences. \\
\textbf{Deployment} & Monolithic (same codebase as bot). & Distributed/Modular (separate process/service). \\
\textbf{Primary Use} & Single-purpose bots or scripts. & Complex ecosystems, "plug and play" data. \\
\hline
\end{tabular}%
}
}
...
\end{frame}
% ================================================================================
\section{MARRVEL-MCP: A Context-Engineered Natural-Language Query-to-Response Interface for Mendelian Disease Discovery}
% ================================================================================

\begin{frame}
\frametitle{What is \href{https://marrvel.org/}{MARRVEL}?}
\begin{columns}[T]
\begin{column}{0.40\textwidth}
    \textbf{MARRVEL}:
    {\scriptsize
    \begin{itemize}
        \item \textbf{Aggregated Resources}: Integration of human databases (OMIM, ExAC, ClinVar, etc.) and model organism databases (FlyBase, MGI, SGD, etc.).
        \item \textbf{Variant Prioritization}: Facilitates using cross-species alignments to prioritize rare human gene variants.
    \end{itemize}
    }
    ...
    {\scriptsize
    \begin{itemize}
        \item \textbf{Identifier-Only}: Historically, searches are restricted to specific identifiers (gene symbols, protein IDs) of genes/variants.
    \end{itemize}
    }
\end{column}
\begin{column}{0.70\textwidth}
    \begin{center}
    \includegraphics[width=\textwidth]{figures/marrvel-landing.png}
    \end{center}
\end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
    \begin{column}{0.4\textwidth}
        ...
        \begin{itemize}
        \item \textbf{What MARRVEL-MCP offers} Enables clinicians/researchers to query 35+ genetics databases using plain English.
